...
public class Timer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (solveActive){
            ...
            if (showTimerToggle.isOn){
                ...
                if (solves.Count() == 0 || solves.Count() == 1){
                    timerText.color = Color.white;
                }
                else if (timeCounter < PB){
                    timerText.color = Color.green;
                }
                else if (timeCounter > AVG){
                    timerText.color = Color.red;
                }
                else {
                    timerText.color = Color.white;
    ...
    void StopTimer(){
        ...
        if (solves.Count() >= 1){
            Solve lastSolve = solves.Last();
            float lastSolveTime = lastSolve.solveTime;

            float lastSolveDelta = timeCounter - lastSolveTime;
            if (lastSolveDelta < 0) {
                timeDeltaText.color = Color.green;
                timeDeltaText.text = String.Format("({0:0.00})", lastSolveDelta);
            }
            else if (lastSolveDelta > 0) {
                timeDeltaText.color = Color.red;
                timeDeltaText.text = String.Format("(+{0:0.00})", lastSolveDelta);
    ...
    void HandleTimeList(){
        ...
        int solve_count = Math.Max(0, solves.Count() - timeListCutoff) + 1;
