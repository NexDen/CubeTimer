...
public class Timer : MonoBehaviour
{
    ...
    */
    ...
    [SerializeField] List<Text> kapatılacaklar_yazı;
    [SerializeField] List<Button> kapatılacaklar_buton;
    ...
    void ZamanlayıcıyıDurdur() {
        foreach (Text t in kapatılacaklar_yazı)
        {
            t.enabled = true;
        }
        foreach (Button b in kapatılacaklar_buton)
        {
            b.enabled = true;
        }
        _başladı = false;
        ...
        if (eniyiZaman == 0 || _zaman < eniyiZaman){
            eniyiZaman = _zaman;
        }
        _eniyiZaman.text = ZamanıSaateÇevir(_zaman);
